# -*- coding: utf-8 -*-
"""Methods to process easy-reading raw strings of GraphParser input."""
...
def _process_settings(data):
    """Process easy-reading raw settings."""

    processed_settings = {
        'tokens': data.get('tokens'),
        'rules': _process_rules(data.get('rules')),
        'onmatch_rules': _process_onmatch_rules(data.get('onmatch_rules')),
...
def _process_onmatch_rules(raw_onmatch_rules):
    ...
    for item in raw_onmatch_rules:
        ...
        _prev_classes = _ONMATCH_CLASS_RE.findall(prev_str)
        _next_classes = _ONMATCH_CLASS_RE.findall(next_str)
